...
class ContributionRecipientsImportTest extends TestCase
{
    ...
    {
        $event = $this->event();
        $import = new ContributionRecipientsImport($this->campaign($event)->id);
    ...
    private function campaign(Event $event): ContributionCampaign
    {
        return ContributionCampaign::query()->create([
            'event_id' => $event->id,
