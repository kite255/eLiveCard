<x-filament-panels::page>
        ...
        x-data="{
            ...
            async generate(regenerate = false) {
                try {
                    ...
                    await this.copyLink(false);
                } catch (error) {
                    console.error('Unable to generate RSVP link:', error);
                    this.notify(error.message ?? 'Unable to generate the client link.', 'danger');
                }
            },

            async copyWithFallback(value) {
                if (
                    navigator.clipboard
            ...
            async copyLink(generateWhenMissing = true) {
                if (! this.url) {
                ...
                try {
                    await this.copyWithFallback(this.url);
            ...
            async disable() {
                ...
                try {
                    ...
                    this.notify(error.message ?? 'Unable to disable the client link.', 'danger');
                }
            },
        }"
    ...
    >
        <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
                <h3 class="text-sm font-bold text-gray-950 dark:text-white">Client RSVP Report Link</h3>
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Share a secure, read-only RSVP dashboard with the client.</p>

                <div class="mt-3 flex flex-wrap gap-4">
                    <label class="inline-flex items-center gap-2 text-xs font-medium text-gray-600 dark:text-gray-300">
                        <input type="checkbox" x-model="showPhone" class="rounded border-gray-300 text-primary-600 focus:ring-primary-500">
                        Show phone numbers
                    </label>

                    <label class="flex items-center gap-2 text-xs font-medium text-gray-600 dark:text-gray-300">
                        Expires after
                        <input type="number" min="1" max="365" x-model="expiresInDays" placeholder="No expiry" class="w-28 rounded-lg border-gray-300 text-xs dark:border-white/10 dark:bg-gray-800">
                        days
                    </label>
                </div>
            </div>
            ...
            <div class="flex flex-wrap gap-2">
                <x-filament::button type="button" color="primary" icon="heroicon-o-clipboard" x-on:click.prevent.stop="copyLink()" x-bind:disabled="loading">
                    <span x-text="copied ? 'Copied!' : (loading ? 'Working...' : 'Copy Client Link')">Copy Client Link</span>
                </x-filament::button>

                <x-filament::button type="button" color="warning" outlined icon="heroicon-o-arrow-path" x-on:click.prevent.stop="generate(true)" x-bind:disabled="loading">
                    <span x-text="loading ? 'Working...' : 'Regenerate'">Regenerate</span>
                </x-filament::button>

                <x-filament::button type="button" color="danger" outlined icon="heroicon-o-link-slash" x-show="enabled" x-cloak x-on:click.prevent.stop="disable()" x-bind:disabled="loading">
                    Disable
                </x-filament::button>
            </div>
        </div>

        <div x-show="url" x-cloak class="mt-4 rounded-xl border border-gray-200 bg-gray-50 p-3 dark:border-white/10 dark:bg-white/5">
            <div class="flex items-start justify-between gap-3">
                <p class="min-w-0 flex-1 break-all text-xs font-medium text-gray-600 dark:text-gray-300" x-text="url"></p>
                <button type="button" class="shrink-0 rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-xs font-bold text-[#213B73] hover:border-[#213B73] dark:border-white/10 dark:bg-gray-900" x-on:click.prevent.stop="copyLink(false)" x-bind:disabled="loading">
